...
The owner requires software-selectable USB, Bluetooth and local Wi-Fi links and declares a PX1122R evaluation board Version 1, ESP32 DevKitC V4, Digi XBee-PRO S3B peer-to-peer-only radios and a SunFounder four-channel 5 V relay board. The matching EVB assembly/schematic, radio firmware/full SKU, relay SKU/thresholds, channel mapping, polarity, controller protocol and electrical qualification remain unverified. [Receiver hardware qualification](receiver-hardware-qualification.md) records current primary-source specifications, required level shifting/buffering, electrical vetoes and bench/field gates. No relay command, pin assignment, native transport or physical accuracy is verified by this plan.
