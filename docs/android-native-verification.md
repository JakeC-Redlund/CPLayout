...
- `EXPO_PUBLIC_CPLAYOUT_ANDROID_NATIVE_PROOF=1 npm run start -w @cplayout/mobile -- --dev-client --port 8082 --clear` serves a proof-only Android development-client bundle. With that server running, `npm run verify:android-native -- --collect --dev-client-url "exp+center-pivot-layout://expo-development-client/?url=http%3A%2F%2F127.0.0.1%3A8082" --wait-ms 30000` launches the app, collects the in-app SQLite/archive proof marker from logcat, drives Files ZIP export/import through Android UIAutomator, captures share-sheet/DocumentsUI screenshots/XML, and writes observed results. The present single-process collector cannot produce a complete native qualification report.
- Use `--serial <adb-serial>` to bind collection to the intended target. Current-process schema/archive results and OS share-sheet/DocumentsUI screenshots are necessary but insufficient: separate restart, fresh-install/upgrade history, and exact installed-source evidence are still required.
...
### Proof Corrections: 2026-09-14

The source review found that a same-process save/list/load had been labeled a completed relaunch, app launch had been labeled install/upgrade evidence, and a current schema snapshot had been labeled migration-history proof. Those inferred passes are removed. The pure evidence assembler preserves observed CRUD, archive and schema data, but marks restart flags false and install/upgrade/panel-inspection checks `not_run`. Its packet carries `executionScope: "single_process"`; even a manually relabeled status/checklist cannot pass the completion validator while that marker remains. Seventeen focused tests passed, including an intentional pre-fix false-completion regression. Legacy reports without this optional marker remain parseable assertions, not independently authenticated evidence.

The collector still uses the ordinary project repository and a fixed proof-project ID. Run it only in an isolated commissioning environment after preserving app/AVD data, APKs and signing material. It is not a non-mutating production-database diagnostic. Two-process restart, separate fresh/upgrade baselines, proof-database isolation, build-to-bundle identity and exported-to-picked ZIP identity remain implementation gates. Historical report validation is not reclassified as directly witnessed runtime proof.

Host preparation preserved the `cplayout_api36` AVD, registration, debug keystore and prior APK under `/home/cyber/cplayout-native-proof-20260914-rMTCEq/`; the AVD copy compared equal before any launch. The existing Gradle 9.0.0 wrapper now pins the [official distribution checksum](https://services.gradle.org/distributions/gradle-9.0.0-bin.zip.sha256), `8fad3d78296ca518113f3d29016617c7f9367dc005f932bd9d93bf45ba46072b`. Its download/checksum verification and `./gradlew --version` succeeded with JDK 17.0.20. This does not prove an APK build, installed application, native SQLite/ZIP, receiver interface, relay contact behavior or 3D accuracy. No receiver endpoint was established by the read-only hardware inventory; [hardware qualification](receiver-hardware-qualification.md) records the remaining electrical and physical gates.

The bounded Gradle build subsequently exposed a source-packaging defect: tracked `ic_launcher_background.xml` references five local splash PNG variants that the blanket `*.png` ignore rule excluded from snapshots. A narrowly scoped exception now includes those unchanged existing assets, and `tools/androidResources.test.ts` checks resolution, density dimensions, decoded image content and ignore eligibility. Original artwork-generation provenance is unknown. The repaired source snapshot and resource-link attempt are recorded in [full refactor execution](full-refactor-execution.md); a passed resource check is not an APK or runtime pass. All build attempts preserve their distinct failure/interruption outcomes, and no emulator/app-data mutation is implied by building.

The later bounded CMake build and installation succeeded on a new isolated API-36 emulator. Installed APK SHA-256 is `f8997e44b6a7e185c470090e94cb07cf1d2d680b91f5196d16945aebae666243`. The source-matched collector observed current-process schema-v11 SQLite CRUD/settings/delete and an in-memory ZIP round trip, but exited 1: a System UI ANR and Expo onboarding blocked Files navigation. Preserve that failed report and screenshots under `/home/cyber/cplayout-native-proof-20260914-rMTCEq/native-runtime/`. Dismissing those overlays is preparation for another attempt, not a repair to the failed evidence. The frozen native source predates the latest draft/cache changes, and the served development bundle hash alone does not prove which bytes the device consumed.

Collector review found overly broad share/picker classifiers and selected-file fields populated from a host fixture after only attempting a tap. Those fields are not receiver-side evidence. A captured picker, successful tap, loaded project, received file bytes, and an exported-to-imported ZIP identity comparison are distinct observations. The repaired collector passed 21 focused tests and independent review, including mixed-package overlays, preexisting fixture breadcrumbs, disabled/failed taps and stale dumps. It uses a full-UUID fixture, captures before tapping, keeps receipt fields empty/zero, and supports only the observed historical Samsung share profile. The fresh catalog still exposes neither ZIP control before a project is opened; a bounded blank-design bootstrap supplies that prerequisite. The repair is not a current-device run and does not supply restart, upgrade, database-isolation, received-file or field evidence.
