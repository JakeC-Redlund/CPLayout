import { Inflate, strFromU8, strToU8, ZipPassThrough, zipSync } from "fflate";
import { z } from "zod";
...
/** ZIP32, one disk, regular stored/deflated files; descriptors may include or omit their signature.
 * Record layouts: https://pkware.cachefly.net/webdocs/casestudies/APPNOTE.TXT (4.3 and 4.4).
 * fflate's unzip filter omits CRC, flags and offsets, so validate records before allocating output.
 */
function inspectZip(bytes: Uint8Array): ZipEntry[] {
  ...
  assertZip(count <= DESIGN_DRAFT_ARCHIVE_MAX_FILE_COUNT, "file-count budget exceeded");
  assertZip(count === filenames.length, "must contain exactly manifest.json and draft.json");
...
function assertZip(condition: boolean, message: string): asserts condition {
  if (!condition) throw new Error(`Draft archive ${message}.`);
}
...
function requireFiles(names: string[]): void {
  if (names.length !== filenames.length || filenames.some((name) => !names.includes(name))) {
    throw new Error("Draft archive must contain exactly manifest.json and draft.json.");
  }
}

function validateFilename(name: string): void {
  if (!filenames.some((filename) => filename === name)) throw new Error(`Draft archive contains unexpected file: ${name}.`);
}

function validatePath(name: string): void {
  if (!name || name.length > 180 || name.includes("\\") || /^[A-Za-z]:/.test(name)
    || name.split("/").some((part) => !part || part === "." || part === "..")) {
    throw new Error(`Draft archive contains unsafe path: ${name}.`);
  }
}
